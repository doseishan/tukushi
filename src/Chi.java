...
//主に自分が書いたコードじゃない部分はここに分離
//基本的に.chiファイル(暗号化と復号の処理を行うコードで。これを含むとGPLになる

public class Chi {
	// テキストファイル読み込み(暗号化ファイルも復号化して)Stringを返却
	public static String readCipheredFile(String strFilePath,
			String charsetName, String key) throws Exception {
		String strText = null;
	...
	private static String setBinTextToStrText(byte[] binText, String charsetName)
			throws Exception {
		// バイナリデータをstrTextにセット
	...
	// テキストファイル書込処理
	// public static void writeTextFile(String strFilePath, String text) throws
	// Exception
	public static void writeTextFile(String strFilePath, byte[] text)
			throws Exception {
	...
	// public static String decrypt(byte[] data, byte[] passDigest) throws
	// Exception
	public static byte[] decrypt(byte[] data, byte[] passDigest)
			throws Exception {
		...
		for (int i = 4; i < 8; i++) {
			...
			if (size < 0)
				size = 256 + size;
			// System.out.println( "check size:"+bytedata.toString()+ " "+ size
			// + " "+k);
	...
	public static byte[] encrypt(byte[] data, byte[] passDigest)
			throws Exception {
		int enc_size;
		if (data.length % 8 == 0) {
		...
		// 4-7 : data length (include randum area + md5sum)(4 bytes)
		/***************
		 * int k = 1*256; int l = data.length; int j = l % k; savedata[4]= new
		 * Integer(j).byteValue(); for(int i=5;i<8;i++){ //
		 * System.out.println("k="+k+",size="+l); l = l / k; j = l % k;
 ...
 */
final class Blowfish extends Cipher {
	...
	public void setKey(byte[] key) {
		...
		/**
		 * for (j = 0, i = 0; i < 16 + 2; i++) { temp = (((key[(j + 3) % len] &
		 * 0xff) << 24) | ((key[(j + 2) % len] & 0xff) << 16) | ((key[(j + 1) %
		 * len] & 0xff) << 8) | ((key[j] & 0xff))); P[i] = P[i] ^ temp; j = (j +
		 * 4) % len; }
		 **/
	...
	public void encrypt(int xL, int xR, int[] out) {
		 ...
		 * 0xff)]) ^ S2[(int) ((L >>> 16) & 0xff)]) + S3[(int) ((L >>> 24) &
		 * 0xff)]) ^ P[1]); L ^= ((((S0[(int) (R & 0xff)] + S1[(int) ((R >>> 8)
		 * & 0xff)]) ^ S2[(int) ((R >>> 16) & 0xff)]) + S3[(int) ((R >>> 24) &
		 * 0xff)]) ^ P[2]); R ^= ((((S0[(int) (L & 0xff)] + S1[(int) ((L >>> 8)
		 * & 0xff)]) ^ S2[(int) ((L >>> 16) & 0xff)]) + S3[(int) ((L >>> 24) &
		 * 0xff)]) ^ P[3]); L ^= ((((S0[(int) (R & 0xff)] + S1[(int) ((R >>> 8)
		 * & 0xff)]) ^ S2[(int) ((R >>> 16) & 0xff)]) + S3[(int) ((R >>> 24) &
		 * 0xff)]) ^ P[4]); R ^= ((((S0[(int) (L & 0xff)] + S1[(int) ((L >>> 8)
		 * & 0xff)]) ^ S2[(int) ((L >>> 16) & 0xff)]) + S3[(int) ((L >>> 24) &
		 * 0xff)]) ^ P[5]); L ^= ((((S0[(int) (R & 0xff)] + S1[(int) ((R >>> 8)
		 * & 0xff)]) ^ S2[(int) ((R >>> 16) & 0xff)]) + S3[(int) ((R >>> 24) &
		 * 0xff)]) ^ P[6]); R ^= ((((S0[(int) (L & 0xff)] + S1[(int) ((L >>> 8)
		 * & 0xff)]) ^ S2[(int) ((L >>> 16) & 0xff)]) + S3[(int) ((L >>> 24) &
		 * 0xff)]) ^ P[7]); L ^= ((((S0[(int) (R & 0xff)] + S1[(int) ((R >>> 8)
		 * & 0xff)]) ^ S2[(int) ((R >>> 16) & 0xff)]) + S3[(int) ((R >>> 24) &
		 * 0xff)]) ^ P[8]); R ^= ((((S0[(int) (L & 0xff)] + S1[(int) ((L >>> 8)
		 * & 0xff)]) ^ S2[(int) ((L >>> 16) & 0xff)]) + S3[(int) ((L >>> 24) &
		 * 0xff)]) ^ P[9]); L ^= ((((S0[(int) (R & 0xff)] + S1[(int) ((R >>> 8)
		 * & 0xff)]) ^ S2[(int) ((R >>> 16) & 0xff)]) + S3[(int) ((R >>> 24) &
		 * 0xff)]) ^ P[10]); R ^= ((((S0[(int) (L & 0xff)] + S1[(int) ((L >>> 8)
		 * & 0xff)]) ^ S2[(int) ((L >>> 16) & 0xff)]) + S3[(int) ((L >>> 24) &
		 * 0xff)]) ^ P[11]); L ^= ((((S0[(int) (R & 0xff)] + S1[(int) ((R >>> 8)
		 * & 0xff)]) ^ S2[(int) ((R >>> 16) & 0xff)]) + S3[(int) ((R >>> 24) &
		 * 0xff)]) ^ P[12]); R ^= ((((S0[(int) (L & 0xff)] + S1[(int) ((L >>> 8)
		 * & 0xff)]) ^ S2[(int) ((L >>> 16) & 0xff)]) + S3[(int) ((L >>> 24) &
		 * 0xff)]) ^ P[13]); L ^= ((((S0[(int) (R & 0xff)] + S1[(int) ((R >>> 8)
		 * & 0xff)]) ^ S2[(int) ((R >>> 16) & 0xff)]) + S3[(int) ((R >>> 24) &
		 * 0xff)]) ^ P[14]); R ^= ((((S0[(int) (L & 0xff)] + S1[(int) ((L >>> 8)
		 * & 0xff)]) ^ S2[(int) ((L >>> 16) & 0xff)]) + S3[(int) ((L >>> 24) &
		 * 0xff)]) ^ P[15]); L ^= ((((S0[(int) (R & 0xff)] + S1[(int) ((R >>> 8)
		 * & 0xff)]) ^ S2[(int) ((R >>> 16) & 0xff)]) + S3[(int) ((R >>> 24) &
		 * 0xff)]) ^ P[16]); R ^= P[17];
		 **/
	...
	public int[] decrypt(int xL, int xR, int[] out) {
		 ...
		 * 0xff)]) ^ S2[(int) ((L >>> 16) & 0xff)]) + S3[(int) ((L >>> 24) &
		 * 0xff)]) ^ P[16]); L ^= ((((S0[(int) (R & 0xff)] + S1[(int) ((R >>> 8)
		 * & 0xff)]) ^ S2[(int) ((R >>> 16) & 0xff)]) + S3[(int) ((R >>> 24) &
		 * 0xff)]) ^ P[15]); R ^= ((((S0[(int) (L & 0xff)] + S1[(int) ((L >>> 8)
		 * & 0xff)]) ^ S2[(int) ((L >>> 16) & 0xff)]) + S3[(int) ((L >>> 24) &
		 * 0xff)]) ^ P[14]); L ^= ((((S0[(int) (R & 0xff)] + S1[(int) ((R >>> 8)
		 * & 0xff)]) ^ S2[(int) ((R >>> 16) & 0xff)]) + S3[(int) ((R >>> 24) &
		 * 0xff)]) ^ P[13]); R ^= ((((S0[(int) (L & 0xff)] + S1[(int) ((L >>> 8)
		 * & 0xff)]) ^ S2[(int) ((L >>> 16) & 0xff)]) + S3[(int) ((L >>> 24) &
		 * 0xff)]) ^ P[12]); L ^= ((((S0[(int) (R & 0xff)] + S1[(int) ((R >>> 8)
		 * & 0xff)]) ^ S2[(int) ((R >>> 16) & 0xff)]) + S3[(int) ((R >>> 24) &
		 * 0xff)]) ^ P[11]); R ^= ((((S0[(int) (L & 0xff)] + S1[(int) ((L >>> 8)
		 * & 0xff)]) ^ S2[(int) ((L >>> 16) & 0xff)]) + S3[(int) ((L >>> 24) &
		 * 0xff)]) ^ P[10]); L ^= ((((S0[(int) (R & 0xff)] + S1[(int) ((R >>> 8)
		 * & 0xff)]) ^ S2[(int) ((R >>> 16) & 0xff)]) + S3[(int) ((R >>> 24) &
		 * 0xff)]) ^ P[9]); R ^= ((((S0[(int) (L & 0xff)] + S1[(int) ((L >>> 8)
		 * & 0xff)]) ^ S2[(int) ((L >>> 16) & 0xff)]) + S3[(int) ((L >>> 24) &
		 * 0xff)]) ^ P[8]); L ^= ((((S0[(int) (R & 0xff)] + S1[(int) ((R >>> 8)
		 * & 0xff)]) ^ S2[(int) ((R >>> 16) & 0xff)]) + S3[(int) ((R >>> 24) &
		 * 0xff)]) ^ P[7]); R ^= ((((S0[(int) (L & 0xff)] + S1[(int) ((L >>> 8)
		 * & 0xff)]) ^ S2[(int) ((L >>> 16) & 0xff)]) + S3[(int) ((L >>> 24) &
		 * 0xff)]) ^ P[6]); L ^= ((((S0[(int) (R & 0xff)] + S1[(int) ((R >>> 8)
		 * & 0xff)]) ^ S2[(int) ((R >>> 16) & 0xff)]) + S3[(int) ((R >>> 24) &
		 * 0xff)]) ^ P[5]); R ^= ((((S0[(int) (L & 0xff)] + S1[(int) ((L >>> 8)
		 * & 0xff)]) ^ S2[(int) ((L >>> 16) & 0xff)]) + S3[(int) ((L >>> 24) &
		 * 0xff)]) ^ P[4]); L ^= ((((S0[(int) (R & 0xff)] + S1[(int) ((R >>> 8)
		 * & 0xff)]) ^ S2[(int) ((R >>> 16) & 0xff)]) + S3[(int) ((R >>> 24) &
		 * 0xff)]) ^ P[3]); R ^= ((((S0[(int) (L & 0xff)] + S1[(int) ((L >>> 8)
		 * & 0xff)]) ^ S2[(int) ((L >>> 16) & 0xff)]) + S3[(int) ((L >>> 24) &
		 * 0xff)]) ^ P[2]); L ^= ((((S0[(int) (R & 0xff)] + S1[(int) ((R >>> 8)
		 * & 0xff)]) ^ S2[(int) ((R >>> 16) & 0xff)]) + S3[(int) ((R >>> 24) &
		 * 0xff)]) ^ P[1]); R ^= P[0];
		 **/
